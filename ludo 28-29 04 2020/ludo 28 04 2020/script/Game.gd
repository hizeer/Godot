...
func SetPlateau():
	...
	functocall = "SetBoats"
	rpc("setp2global",max_size, infosGlobales.infosPartie["dureeTour"])
...
func SetBoats():
	for player in _joueurs:
		...
		for bat in player.Boats:
			print(bat.name)
			bat.rpc("remoteMoveBoat",bat._position.x + x,bat._position.y + y)
...
sync func RemovePlayer(j):
	if j != _joueurActuel:
		...
		if infosGlobales.isServer:
			rpc("SetJoueurActuel",JoueurSuivant(j).id)
		_joueurs.erase(j)
	nbrJoueurs -= 1
