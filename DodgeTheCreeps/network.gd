...
func _connexion_serveur_reussi():
	...
	# Demande au serveur d'enregistrer le nouveau joueur en le donnat aux autres joueurs
...
remote func enregistrer_joueur(pinfo):
	if (get_tree().is_network_server()):
		...
		for id in players:
			...
			rpc_id(pinfo.net_id,"joueur_enregistre",players[id])
			...
			if(id != 1):
				rpc_id(id, "joueur_enregistre",pinfo)
