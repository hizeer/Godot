...
func Set_Posable(val):
	posable = val
	if val:
		$Hitbox/ReferenceRect.modulate.a = 80
	else:
		 $Hitbox/ReferenceRect.modulate.a = 0
...
func HoverEnter():
	...
	if GlobalLudo.You.destroying and !hit: #si on est en train de détruire une case affiche la case selectionner
...
func CasesPosables(val,distance):
	...
	if distance > 0:
		...
		if CaseB:
			CaseB.CCasesPosables(val,distance-1)
		if CaseD:
